...
public static class RepeatCommand {
    ...
    public static int Run(string[] args, TextReader stdin, TextWriter output, TextWriter error) {
        ...
        for (int i = 0; i < strings.Count; i++) {
            ...
            if (max >= 0 && repeated.Length > max) repeated = repeated[..max];
            if (repeated.Length > 0) changes = true;
            if (!quiet) {
                bool isLast = i == strings.Count - 1;
                if (noNewline && isLast) output.Write(repeated);
                else output.WriteLine(repeated);
            }
        }
